...
>(null);
  ...
  try {
    ...
    const newRegs = registrations.map((r) => {
      ...
      if (type === "generatedLetter") {
        return {
          ...r,
          generatedLetters: r.generatedLetters.filter((g) => g._id !== id),
        };
      }
          ...
          <ModalContent maxW={{ base: "94vw", md: "800px" }} mx="auto" maxH="90vh" overflowY="auto">
            ...
            <ModalBody>
              <VStack align="start" spacing={4} fontSize="sm">
                ...
                {/* 📌 Initiated Letters (generated via Initiate Letters) */}
                <Box w="full">
                  <Text fontWeight="bold">Initiated Letters</Text>
                  {selectedCompany.generatedLetters.length > 0 ? (
                    selectedCompany.generatedLetters.map((gl) => (
                      <Flex
                        key={gl._id}
                        p={3}
                        borderWidth="1px"
                      ...
                      >
                        <Box>
                          <Text>Type: {gl.letterType}</Text>
                          <Text>Title: {gl.title}</Text>
                          <Text>Reporting Date: {gl.reportingDate}</Text>
                          <Text fontSize="xs" color="gray.500">
                            Generated by: {gl.generatedBy || "N/A"} —{" "}
                            {new Date(gl.createdAt).toLocaleString()}
                          </Text>
                        </Box>
                        <HStack>
                          <Button
                            size="xs"
                            colorScheme="red"
                            onClick={() => handleDelete("generatedLetter", gl._id)}
                          >
                            Delete
                          </Button>
                        </HStack>
